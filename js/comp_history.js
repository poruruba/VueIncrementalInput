...
<div class="dropdown input-group">
  <input class="form-control" type="text" data-toggle="dropdown" v-model="input_text" v-on:input="input_change">
  <ul class="dropdown-menu">
    <li v-for="(item, index) in item_list" value="item" v-on:click="item_select(index)">
      <a style="padding-right: 0px;">{{item}} <span class="pull-right" v-on:click.stop="call_item_delete(index)">&nbsp;&nbsp;&nbsp; × &nbsp;&nbsp;</span></a>
    </li>
  </ul>
  <span class="input-group-addon" v-on:click="input_clear">×</span>
</div>
`,
  data: function(){
    return {
